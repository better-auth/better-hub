...
repoCommand
	...
	.action(async (opts: { slug?: string; description?: string; visibility: string }) => {
		...
		const { description, name, path, segment, slug, visibility } = await group(
			{
				name: () => {
					return text({
		...
		const commands: [string, string][] = [
			...
			["bh push -m", "Commit with an AI-generated message"],
			["bh repo list", "List your repositories"],
		];
